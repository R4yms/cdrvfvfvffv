...
if(![(Roles.Registerer)].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.reply(`**__Bu Komut İçin Yetkiniz Bulunmamaktadır.__**`) 
  ...
  const isim = args[1];
  const yas = args[2];
  const chat = message.guild.channels.cache.find(r => r.id === (Channels.Channels.GeneralChat))
  const logg = message.guild.channels.cache.find(r => r.id === (Channels.Channels.RegisterChat))
...
Erkek ise ♂ Emojisine, Kadın ise ♀ Emojisine Basın.`)
  ...
  .setColor("#323131")
  logg.send(embedd).then(async mesaj => {
    await mesaj.react('♂') 
    await mesaj.react('♀')
