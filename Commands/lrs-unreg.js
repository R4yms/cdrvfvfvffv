...
exports.run = async (client, message, args) => {
  ...
  if(!users) return message.channel.send(embedx.setDescription(`**__Bir Üye Etiketlemelisin.**__`).setFooter(`${dev} ${y}${s}${r}${a}${l}`).setTimestamp().setColor("#323131"))

//users.setNickname(users.user.username)
users.setNickname(ServerSettings.WelcomeName)
users.setNickname(ServerSettings.WelcomeName)
